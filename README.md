...
The fastest full-adder schedule the compiler produces — **17 steps / 11
cells** with `--unlimited-cells` (rows = crossbar cells, columns = pulses;
`⊥` = FALSE reset, boxes = IMPLY). Note how all resets merge into a single
upfront FALSE pulse in step 1:

![full adder 17-step schedule (--unlimited-cells)](docs/assets/full_adder/full_adder_schedule_unlimited.svg)

The default mode instead minimizes the row width (20 steps / 7 cells — see
the side-by-side comparison in §4). Every compiled circuit gets this diagram
automatically, next to its `.seq.txt`, in `synthesis/compiled/<circuit>/`.
