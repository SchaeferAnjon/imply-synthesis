...
The 20-step full-adder schedule, exactly as generated by the compiler in
default mode (rows = crossbar cells, columns = pulses; `⊥` = FALSE reset,
boxes = IMPLY):

![full adder 20-step schedule (default)](docs/assets/full_adder/full_adder_schedule.svg)

Every compiled circuit gets this diagram automatically, next to its
`.seq.txt`, in `synthesis/compiled/<circuit>/`. Faster variants of this same
schedule are one flag away — see §4.

## 4. Scheduling modes: two opt-in switches

Two independent opt-in switches trade row width against pulse count:
`--unlimited-cells` gives every value a fresh cell, so all resets merge into
**one upfront FALSE pulse** (fewest steps); `--preserve-inputs` keeps every
input cell readable to the end (the supervisor-requested corner). All four
combinations on the full adder:

| default — **20 steps / 7 cells** | `--preserve-inputs` — **20 steps / 10 cells** |
|---|---|
| ![default](docs/assets/full_adder/full_adder_schedule.svg) | ![preserve-inputs](docs/assets/full_adder/full_adder_schedule_preserved.svg) |
| **`--unlimited-cells` — 17 steps / 11 cells** | **both switches — 18 steps / 12 cells** |
| ![unlimited-cells](docs/assets/full_adder/full_adder_schedule_unlimited.svg) | ![preserve + unlimited](docs/assets/full_adder/full_adder_schedule_preserved_unlimited.svg) |

Reading the four side by side: the bottom row starts with the single merged
FALSE pulse; the right column keeps `a` / `b` / `cin` untouched to the end
(no box ever lands on their rails), at the cost of a wider row.

```bash
python3 compile.py circuits/arithmetic/full_adder.v [--preserve-inputs] [--unlimited-cells]
```
